...
-include_lib("../include/prelude.hrl").
...
temp_name(Stem)        -> Stem ++ ?i2l(random_n(0, 1 bsl 127)).
...
random_n(Min, Max) when is_integer(Min), is_integer(Max) ->
  Min - 1 + rand:uniform(Max + 1 - Min);
random_n(Min, Max) ->
  Min2 = trunc(Min * 10000),
  Max2 = trunc(Max * 10000),
  random_n(Min2, Max2) / 10000.
