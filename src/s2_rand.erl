...
pick(Xs) -> lists:nth(s2_sh:random_n(1, length(Xs) + 1), Xs).
...
shuffle(Xs) ->
  [X || {_N, X} <- lists:keysort(1, [{s2_sh:random_n(0, length(Xs)), X} ||
                                      X <- Xs])].
